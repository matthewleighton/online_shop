<?php
	class carts_helper {
		...
		public static function alreadyInCart($productId) {
			$sql = 'SELECT cart_id FROM shopping_cart WHERE fk_shopping_cart_user = ? AND fk_shopping_cart_product = ?';
		...
		public static function incrementProductQuantity($productId, $quantity) {
			$sql = "UPDATE shopping_cart SET cart_quantity = cart_quantity + ? " . 
				   "WHERE fk_shopping_cart_user = ? AND fk_shopping_cart_product = ?";
